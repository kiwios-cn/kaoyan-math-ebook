...
window.BOOK_MANIFEST = {
  ...
  "generatedAt": "2026-08-03T19:31:52+08:00",
  "stats": {
    "documents": 19,
    "pages": 190,
    "renderDpi": 260,
...
window.SEARCH_INDEX = [
  ...
  },
  {
    ...
    "category": "微积分",
    "page": 6,
    "heading": "2. lim𝑥→𝑥0 𝑓(𝑥) 存在。",
    "snippet": "考研数学 · 函数的极限与连续 2. lim𝑥→𝑥0 𝑓(𝑥) 存在。 3. 极限值等于函数值。 2. 间断点分类 类型 判断 处理重点 可去间断点 极限存在，但函数值不存在或不等于极限 补定义使连续 跳跃间断点 左右极限都存在但不相等 分段函数常见 无穷间断点 至少一侧极限为无穷大 检查分母为零处 振荡间断点 极限振荡不存在 如 sin...",
    "text": "Limits and Continuity (函数的极限与连续) Limits and Continuity 函数的极限与连续 微积分 2. lim𝑥→𝑥0 𝑓(𝑥) 存在。 考研数学 · 函数的极限与连续 2. lim𝑥→𝑥0 𝑓(𝑥) 存在。 3. 极限值等于函数值。 2. 间断点分类 类型 判断 处理重点 可去间断点 极限存在，但函数值不存在或不等于极限 补定义使连续 跳跃间断点 左右极限都存在但不相等 分段函数常见 无穷间断点 至少一侧极限为无穷大 检查分母为零处 振荡间断点 极限振荡不存在 如 sin( 𝑥1 ) 类结构 3. 闭区间连续函数性质 若 𝑓(𝑥) 在闭区间 [𝑎, 𝑏] 上连续，则： 1. 有界性：𝑓(𝑥) 在 [𝑎, 𝑏] 上有界。 2. 最值定理：存在最大值和最小值。 3. 介值定理：函数能取到最大值与最小值之间的一切值。 4. 零点定理：若 𝑓(𝑎)𝑓(𝑏) < 0，则存在 𝜉 ∈ (𝑎, 𝑏) 使 𝑓(𝜉) = 0。 五、典型例题 例题 1：有理化求极限 √ √ 1+𝑥− 1−𝑥 题目 求 lim𝑥→0 𝑥 。 有理化： √ √ 1+𝑥− 1−𝑥 (1 + 𝑥) − (1 − 𝑥) = √ √ 𝑥 𝑥( 1 + 𝑥 + 1 − 𝑥) 2 =√ √ 1+𝑥+ 1−𝑥 所以 √ √ 1+𝑥− 1−𝑥 2 lim = =1 𝑥→0 𝑥 1+1 例题 2：连续性确定参数 题目 设 𝑎𝑥 ( 𝑒 −1 , 𝑥 ≠ 0) 𝑓(𝑥) = { 𝑥 (2, 𝑥 = 0) 求使 𝑓(𝑥) 在 𝑥 = 0 连续的 𝑎。 连续要求 𝑒𝑎𝑥 − 1 lim =2 𝑥→0 𝑥"
  },
  {
    ...
    "page": 7,
    "heading": "例题 3：零点定理",
    "snippet": "考研数学 · 函数的极限与连续 由 𝑒𝑎𝑥 − 1 ≃ 𝑎𝑥，得极限为 𝑎。因此 𝑎=2 例题 3：零点定理 题目 证明方程 𝑥3 + 𝑥 − 1 = 0 在 (0, 1) 内至少有一个实根。 令 𝑓(𝑥) = 𝑥3 + 𝑥 − 1，则 𝑓(𝑥) 在 [0, 1] 上连续，且 𝑓(0) = −1 < 0, 𝑓(1) = 1 > 0 由零点...",
    "text": "Limits and Continuity (函数的极限与连续) Limits and Continuity 函数的极限与连续 微积分 例题 3：零点定理 考研数学 · 函数的极限与连续 由 𝑒𝑎𝑥 − 1 ≃ 𝑎𝑥，得极限为 𝑎。因此 𝑎=2 例题 3：零点定理 题目 证明方程 𝑥3 + 𝑥 − 1 = 0 在 (0, 1) 内至少有一个实根。 令 𝑓(𝑥) = 𝑥3 + 𝑥 − 1，则 𝑓(𝑥) 在 [0, 1] 上连续，且 𝑓(0) = −1 < 0, 𝑓(1) = 1 > 0 由零点定理，存在 𝜉 ∈ (0, 1)，使 𝑓(𝜉) = 0 故方程在 (0, 1) 内至少有一个实根。"
  },
  {

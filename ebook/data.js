...
window.SEARCH_INDEX = [
  ...
  },
  {
    ...
    "page": 3,
    "heading": "五、递推数列",
    "snippet": "考研数学 · 数列的极限 这使洛必达法则、泰勒展开和等价无穷小也能服务数列极限。 五、递推数列 递推数列通常形如 𝑎𝑛+1 = 𝜑(𝑎𝑛 ) 常用步骤为： 1. 猜测或求不动点：令 𝐴 = 𝜑(𝐴)。 2. 证明数列有界。 3. 证明数列单调，或证明相邻项差趋于零。 4. 若已知极限存在，则代入递推式求 𝐴。 易错点： 不能只把极限代入递...",
    "text": "Sequence Limits (数列的极限) Sequence Limits 数列的极限 微积分 五、递推数列 考研数学 · 数列的极限 这使洛必达法则、泰勒展开和等价无穷小也能服务数列极限。 五、递推数列 递推数列通常形如 𝑎𝑛+1 = 𝜑(𝑎𝑛 ) 常用步骤为： 1. 猜测或求不动点：令 𝐴 = 𝜑(𝐴)。 2. 证明数列有界。 3. 证明数列单调，或证明相邻项差趋于零。 4. 若已知极限存在，则代入递推式求 𝐴。 易错点： 不能只把极限代入递推式就结束。必须先说明极限存在，常用方法就是单调有界定理或压缩映射 思想。 六、典型例题 例题 1：根式数列 √ 题目 求 lim𝑛→∞ ( 𝑛2 + 𝑛 − 𝑛)。 有理化： √𝑛2 + 𝑛 − 𝑛 = √ 𝑛 2 𝑛 +𝑛+𝑛 上下同除以 𝑛： 𝑛 1 √ = 𝑛2 + 𝑛 + 𝑛 √1 + 𝑛1 + 1 所以 1 lim (√𝑛2 + 𝑛 − 𝑛) = 𝑛→∞ 2 例题 2：递推数列 √ 题目 设 𝑎1 = 2，𝑎𝑛+1 = √2 + 𝑎𝑛 ，求 lim 𝑎𝑛 。 先猜极限。若极限为 𝐴，则 √ 𝐴= 2+𝐴 解得 𝐴 = 2 或 𝐴 = −1。因 𝑎𝑛 > 0，只可能为 2。 再说明极限存在。由 𝑎1 < 2，若 𝑎𝑛 < 2，则 √ 𝑎𝑛+1 = √2 + 𝑎𝑛 < 4=2 所以 𝑎𝑛 < 2。又 𝑎𝑛+1 ≥ 𝑎𝑛 ⇔ √2 + 𝑎𝑛 ≥ 𝑎𝑛 ⇔ 2 + 𝑎𝑛 ≥ 𝑎2𝑛"
  },
  {
    ...
    "page": 4,
    "heading": "例题 3：指数型极限",
    "snippet": "考研数学 · 数列的极限 当 0 < 𝑎𝑛 < 2 时成立。因此数列单调递增且有上界 2，故收敛，极限为 2 例题 3：指数型极限 𝑏 题目 求 lim𝑛→∞ 𝑛((1 + 𝑛𝑎 ) − 1)。 当 𝑛 → ∞ 时， 𝑛𝑎 → 0。由 (1 + 𝑥)𝑏 − 1 ≃ 𝑏𝑥 得 𝑎 𝑏 𝑎 (1 + ) −1≃𝑏 𝑛 𝑛 所以 𝑎 𝑏 lim...",
    "text": "Sequence Limits (数列的极限) Sequence Limits 数列的极限 微积分 例题 3：指数型极限 考研数学 · 数列的极限 当 0 < 𝑎𝑛 < 2 时成立。因此数列单调递增且有上界 2，故收敛，极限为 2 例题 3：指数型极限 𝑏 题目 求 lim𝑛→∞ 𝑛((1 + 𝑛𝑎 ) − 1)。 当 𝑛 → ∞ 时， 𝑛𝑎 → 0。由 (1 + 𝑥)𝑏 − 1 ≃ 𝑏𝑥 得 𝑎 𝑏 𝑎 (1 + ) −1≃𝑏 𝑛 𝑛 所以 𝑎 𝑏 lim 𝑛((1 + ) − 1) = 𝑎𝑏 𝑛→∞ 𝑛"
  },
  {

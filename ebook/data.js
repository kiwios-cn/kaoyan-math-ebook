...
window.BOOK_MANIFEST = {
  ...
  "generatedAt": "2026-08-04T10:13:58+08:00",
  "stats": {
    "documents": 19,
    "pages": 268,
    "renderDpi": 260,
...
window.SEARCH_INDEX = [
  ...
  },
  {
    ...
    "text": "Limits and Continuity (函数的极限与连续) Limits and Continuity 函数的极限与连续 微积分 易错点：若 𝑓(𝑎) = 0，不能直接用 𝑓(𝑎)(𝛽 − 𝛼)，因为主项会被压成 0。此时必须展开被积函数，例如 𝑓(𝑡) ≃ 考研数学 · 函数的极限与连续 等价积分法： 若 𝑡 → 0 时 𝑓(𝑡) ≃ 𝐶𝑡𝑘 ，则 𝑥 𝑥 𝐶 𝑘+1 ∫ 𝑓(𝑡)𝑑𝑡 ≃ ∫ 𝐶𝑡𝑘 𝑑𝑡 = 𝑥 0 0 𝑘+1 也就是说，积分会把被积函数的无穷小阶数提高一阶。 若上下限都是函数，例如 𝛽(𝑥) ∫ 𝑓(𝑡)𝑑𝑡 𝛼(𝑥) 且 𝛼(𝑥) → 𝑎, 𝛽(𝑥) → 𝑎，可令 𝐹 ′ (𝑡) = 𝑓(𝑡)，转化为 𝐹 (𝛽(𝑥)) − 𝐹 (𝛼(𝑥)) 若 𝑓 在 𝑎 连续且 𝑓(𝑎) ≠ 0，则 𝛽(𝑥) ∫ 𝑓(𝑡)𝑑𝑡 ≃ 𝑓(𝑎)(𝛽(𝑥) − 𝛼(𝑥)) 𝛼(𝑥) 例如 𝑥 2 ∫ 𝑒𝑡 𝑑𝑡 sin 𝑥 lim =1 𝑥→0 𝑥 − sin 𝑥 2 因为 sin 𝑥 → 0, 𝑥 → 0，且 𝑒𝑡 → 1，所以积分主项就是区间长度 𝑥 − sin 𝑥。 易错点：若 𝑓(𝑎) = 0，不能直接用 𝑓(𝑎)(𝛽 − 𝛼)，因为主项会被压成 0。此时必须展开被积函数，例如 𝑓(𝑡) ≃ 𝐶(𝑡 − 𝑎)𝑘 ，再积分得到高一阶主项。 处理流程： 1. 先判断变限积分是否趋于 0。 2. 分式极限优先尝试洛必达，并使用变限积分求导公式。 3. 若被积函数主项清楚，可直接用等价积分法。 4. 上下限都是函数时，先看上下限是否趋于同一点，再用原函数差或区间长度近似。 5. 被积函数在端点处为 0 时，必须继续展开主项。"
  },
  {

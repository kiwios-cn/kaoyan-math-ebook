...
window.BOOK_MANIFEST = {
  ...
  "generatedAt": "2026-08-04T11:17:35+08:00",
  "stats": {
    "documents": 19,
    "pages": 270,
    "renderDpi": 260,
...
window.SEARCH_INDEX = [
  ...
  },
  {
    ...
    "snippet": "考研数学 · 函数的极限与连续 等价积分法： 若 𝑡 → 0 时 𝑓(𝑡) ≃ 𝐶𝑡𝑘 ，则 𝑥 𝑥 𝐶 𝑘+1 ∫ 𝑓(𝑡)𝑑𝑡 ≃ ∫ 𝐶𝑡𝑘 𝑑𝑡 = 𝑥 0 0 𝑘+1 也就是说，积分会把被积函数的无穷小阶数提高一阶。 若上下限都是函数，例如 𝛽(𝑥) ∫ 𝑓(𝑡)𝑑𝑡 𝛼(𝑥) 且 𝛼(𝑥) → 𝑎, 𝛽(𝑥) → 𝑎，可令 𝐹...",
    "text": "Limits and Continuity (函数的极限与连续) Limits and Continuity 函数的极限与连续 微积分 6. 变限积分中 𝑥 出现位置的分类处理 考研数学 · 函数的极限与连续 等价积分法： 若 𝑡 → 0 时 𝑓(𝑡) ≃ 𝐶𝑡𝑘 ，则 𝑥 𝑥 𝐶 𝑘+1 ∫ 𝑓(𝑡)𝑑𝑡 ≃ ∫ 𝐶𝑡𝑘 𝑑𝑡 = 𝑥 0 0 𝑘+1 也就是说，积分会把被积函数的无穷小阶数提高一阶。 若上下限都是函数，例如 𝛽(𝑥) ∫ 𝑓(𝑡)𝑑𝑡 𝛼(𝑥) 且 𝛼(𝑥) → 𝑎, 𝛽(𝑥) → 𝑎，可令 𝐹 ′ (𝑡) = 𝑓(𝑡)，转化为 𝐹 (𝛽(𝑥)) − 𝐹 (𝛼(𝑥)) 若 𝑓 在 𝑎 连续且 𝑓(𝑎) ≠ 0，则 𝛽(𝑥) ∫ 𝑓(𝑡)𝑑𝑡 ≃ 𝑓(𝑎)(𝛽(𝑥) − 𝛼(𝑥)) 𝛼(𝑥) 例如 𝑥 2 ∫ 𝑒𝑡 𝑑𝑡 sin 𝑥 lim =1 𝑥→0 𝑥 − sin 𝑥 2 因为 sin 𝑥 → 0, 𝑥 → 0，且 𝑒𝑡 → 1，所以积分主项就是区间长度 𝑥 − sin 𝑥。 6. 变限积分中 𝑥 出现位置的分类处理 求极限时，先看 𝑥 出现在积分的哪一部分。常见有三类： 情形 典型形式 核心处理方法 𝛽(𝑥) 只在上下限中 ∫ 𝑓(𝑡)𝑑𝑡 用原函数差 𝐹 (𝛽(𝑥)) − 𝐹 (𝛼(𝑥))，或对变限积 𝛼(𝑥) 分求导；若上下限趋于同一点，优先用区间长 度近似。 𝑏 只在被积函数中 ∫ 𝑓(𝑥, 𝑡)𝑑𝑡 积分区间固定时，先考虑把极限移入积分；若 𝑎 是未定式，则对 𝑓(𝑥, 𝑡) 关于 𝑥 展开，再逐项 积分。 𝛽(𝑥) 上下限和被积函数中 ∫𝛼(𝑥) 𝑓(𝑥, 𝑡)𝑑𝑡 先看区间是否收缩，再看被积函数主项；必要 都有 时使用含参变量积分求导公式。 第一类：𝑥 只在上下限中。 𝛽(𝑥) 𝐼(𝑥) = ∫ 𝑓(𝑡)𝑑𝑡 𝛼(𝑥) 若 𝛼(𝑥) → 𝑎, 𝛽(𝑥) → 𝑎，且 𝑓 在 𝑎 连续，则 𝐼(𝑥) ≃ 𝑓(𝑎)(𝛽(𝑥) − 𝛼(𝑥)) 若 𝑓(𝑎) = 0，需要继续展开。若 𝑓(𝑡) ≃ 𝐶(𝑡 − 𝑎)𝑘 ，则"
  },
  {
  ...
  },
  {
    ...
    "category": "微积分",
    "page": 10,
    "heading": "易错点：若 𝑓(𝑎) = 0，不能直接用 𝑓(𝑎)(𝛽 − 𝛼)，因为主项会被压成 0。此时必须展开被积函数，例如 𝑓(𝑡) ≃",
    "snippet": "考研数学 · 函数的极限与连续 易错点：若 𝑓(𝑎) = 0，不能直接用 𝑓(𝑎)(𝛽 − 𝛼)，因为主项会被压成 0。此时必须展开被积函数，例如 𝑓(𝑡) ≃ 𝐶(𝑡 − 𝑎)𝑘 ，再积分得到高一阶主项。 处理流程： 1. 先判断 𝑥 出现在上下限、被积函数，还是二者同时出现。 2. 只在上下限中：用原函数差、区间长度近似或变限积分求导...",
    "text": "Limits and Continuity (函数的极限与连续) Limits and Continuity 函数的极限与连续 微积分 易错点：若 𝑓(𝑎) = 0，不能直接用 𝑓(𝑎)(𝛽 − 𝛼)，因为主项会被压成 0。此时必须展开被积函数，例如 𝑓(𝑡) ≃ 考研数学 · 函数的极限与连续 易错点：若 𝑓(𝑎) = 0，不能直接用 𝑓(𝑎)(𝛽 − 𝛼)，因为主项会被压成 0。此时必须展开被积函数，例如 𝑓(𝑡) ≃ 𝐶(𝑡 − 𝑎)𝑘 ，再积分得到高一阶主项。 处理流程： 1. 先判断 𝑥 出现在上下限、被积函数，还是二者同时出现。 2. 只在上下限中：用原函数差、区间长度近似或变限积分求导。 3. 只在被积函数中：区间固定时优先考虑极限进积分；未定式则对被积函数关于 𝑥 展开。 4. 二者同时出现：先看区间是否收缩，再看被积函数主项；必要时用莱布尼茨公式。 5. 被积函数在收缩端点处主项为 0 时，必须继续展开主项。"
  },
  {

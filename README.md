...
日常新增问题时，优先判断问题所属章节：若主题明确，直接追加到对应 Typst 源文件；若归属暂时不确定，先标记为待归类并尽快归入正式章节。新增或改写定理、概念时，优先补充“公式化描述”，采用“适用条件 => 数学结论/判定式”的形式，避免只保留文字解释。章节 Typst 更新后运行 `python3 ebook/scripts/build_index.py`，即可让网页目录、页面图片和搜索结果同步最新内容。
